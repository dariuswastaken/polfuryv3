export const addCertificat = async ({ interaction, mongo, targetID, certificat }) => {
    ...
    switch (certificat) {
        ...
        case 'moto':
            ...
            await targetUser.roles.add(['1103417201182113793', '1102707472344559638']);
            break;
        case 'pilot':
            await mongo.updateCertificate(targetID, 'pilot', true);
            await targetUser.roles.add(['1103417201182113793', '1102708810247847987']);
            break;
        case 'highspeed':
            await mongo.updateCertificate(targetID, 'highspeed', true);
            await targetUser.roles.add(['1103417201182113793', '1102706216985178122']);
            break;
        case 'mdt':
...
export const removeCertificat = async ({ interaction, mongo, targetID, certificat }) => {
    const targetUser = await interaction.guild.members.fetch(targetID);
    switch (certificat) {
        case 'radio':
