import { ChannelType, PermissionsBitField, ThreadAutoArchiveDuration } from 'discord.js';
...
export const createSanctionThread = async (
    ...
    const forum = await interaction.guild.channels.cache.get('1205145266349670440');
    ...
    for (let sanc of sanction.sanctions) {
        formattedSanctions.push(`${sanctionKeys[sanc.split(' - ')[0]]} - ${sanc.split(' - ')[1]}`);
    }
    ...
    const embed = await pulsar.discordManager.embeds.createForumThreadEmbed({
        ...
        description: `**Nume sanctionat/a:** ${sanctionedProfile.nume}\n**Callsign:** ${sanctionedProfile.callsign}\n**ID Sanctionat/a:** ${sanctionedProfile.IDServer}\n**ID Discord:** ${sanctionedProfile.IDDiscord}\n\n**Sanctiuni:**\n\`\`\`${formattedSanctions.join(
            '\n'
    ...
    await utils.discord.embeds.sendSuccessEmbed('Thread-ul a fost creat cu succes.', {
        pulsar: pulsar,
        interaction: interaction
    });
};

export const addSanctionToMember = async ({ pulsar, mongo, userID, sanctions }) => {
    const sanctionKeys = {
        ts25: 'Taiere Salariala 25%',
        ts50: 'Taiere Salariala 50%',
        ts75: 'Taiere Salariala 75%',
...
export const createSanctionPrivateChannel = async ({ pulsar, interaction, mongo, sanctionID }) => {
    const sanction = await mongo.getSanction(sanctionID);
    const sanctionedProfile = await mongo.getProfile(sanction.sanctionedID);

    const sanctionedMember = await interaction.guild.members.fetch(sanction.sanctionedID);
