...
export const createUpList = async ({ interaction, pulsar, utils, mongo, week }) => {
    ...
    for (let member of members) {
        ...
        if (member.grad === 'Cadet') {
            if (diffInDays < flags['Cadet'].up.requiredDays && activityGrade !== 10) {
                continue;
            }
            if (
        ...
        if (member.grad === 'Agent') {
            if (diffInDays < flags['Agent'].up.requiredDays && activityGrade !== 10) {
                continue;
            }
            if (
        ...
        if (member.grad === 'Agent Principal') {
            if (diffInDays < flags['Agent Principal'].up.requiredDays && activityGrade !== 10) {
                continue;
            }
            if (
...
export const createOutList = async ({ interaction, pulsar, utils, mongo, week }) => {
    await pulsar.discordManager.embeds.createDefaultEmbed(
    ...
    for (const member of members) {
        ...
        const dataCurenta = new Date();
        const diffInDays = await utils.dayConversion.getDifferenceInDays(dataIntrare, dataCurenta);
