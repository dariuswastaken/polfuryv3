...
export const calculateMinutes = async (startTime, endTime) => {
...
export const getCurrentHour = async () => {
  const date = moment.tz('Europe/Bucharest');
  const hour = date.format('HH:mm');
  return hour.split(':')[0];
};

export const createTimes = async (time_1, time_2) => {
...
export const validateTimeFormat = async (time, format) => {
  let timeFormat;
  switch (format) {
    case 'HH:mm':
