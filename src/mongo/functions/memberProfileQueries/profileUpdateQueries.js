...
export const updateProfileName = async (id, name) => {
    const result = await db.update('Member', { IDDiscord: id }, { $set: { nume: name } });
    return result;
};

export const updateProfileIDServer = async (id, serverID) => {
    const result = await db.update('Member', { IDDiscord: id }, { $set: { IDServer: serverID } });
...
export const updateProfileIDDiscord = async (id, discordID) => {
    const result = await db.update('Member', { IDDiscord: id }, { $set: { IDDiscord: discordID } });

    await db.updateBulk('Activitate', { IDDiscord: id }, { $set: { IDDiscord: discordID } });
    return result;
};
...
export const updateProfileCallsign = async (id, callsign) => {
    const result = await db.update('Member', { IDDiscord: id }, { $set: { callsign: callsign } });
    return result;
};

export const updateProfileRank = async (id, rank) => {
    const result = await db.update('Member', { IDDiscord: id }, { $set: { grad: rank } });
    return result;
};
...
export const addFunc = async (id, func) => {
    const result = await db.update('Member', { IDDiscord: id }, { $push: { functii: func } });
    return result;
};

export const removeFunc = async (id, func) => {
    const result = await db.update('Member', { IDDiscord: id }, { $pull: { functii: func } });
    return result;
};
...
export const removeAvM = async (userID) => {
    await db.update('Member', { IDDiscord: userID }, { $inc: { avertismente: -1 } });
};
