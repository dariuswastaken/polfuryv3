...
export const updateSanctionReason = async (sanctionID, reason) => {
    await db.update('PendingSanction', { sanctionID: sanctionID }, { $set: { reason: reason } });
};

export const closeSanctionList = async (sanctionID) => {
    await db.update('PendingSanction', { sanctionID: sanctionID }, { $set: { pending: false } });
};

export const closeSanction = async (sanctionID) => {
    await db.update('PendingSanction', { sanctionID: sanctionID }, { $set: { active: false } });
};
